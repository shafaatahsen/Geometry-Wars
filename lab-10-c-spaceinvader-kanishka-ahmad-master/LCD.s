...
; As part of Lab 10, students need to implement these LCD_WriteCommand and LCD_WriteData
...
WCcheck	LDR R2, [R3]
		...
		LDR R3, =DC ;Load PA6 the Data/Command
		LDR R2, [R3]
		BIC R2, #0x40  ;clear PA6 for sending commands
		STR R2, [R3]
		
		LDR R3, =SSI0_DR_R ;write command to LCD
		STRB R0, [R3]
		
		LDR R3, =SSI0_SR_R
WCcheck2 LDR R2, [R3]       ; check SR to see if busy
		ANDS R1, R2, #0x10
		BNE WCcheck2		; if busy repeat
...
;4) Write the 8-bit data to SSI0_DR_R
  		LDR R1, =SSI0_SR_R
WDCheck	LDR R2, [R1]       ;check is SR is high
		ANDS R3, R2, #0x2  ; check if FIFO not full, if full keep checking
		BEQ WDCheck
		
		LDR R1, =DC ;; set D/C PA6 to 1 for Data
		LDR R2, [R1]
		ORR R2, #0x40
		STR R2, [R1]
		
		LDR R1, =SSI0_DR_R ; write data to DR
		STRB R0, [R1]
